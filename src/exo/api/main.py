...
def _memory_evidence_for_selected_workers(
    ...
    *,
    ...
    /instance must validate the workers selected by the reviewed instance
    payload, not unrelated cluster nodes. Missing or non-positive selected-worker
    ram_available is unsafe because it means the live worker did not report
    usable MLX/unified RAM; callers should fail closed before dispatching a
    ...
    for node_id, runner_id in instance.shard_assignments.node_to_runner.items():
        ...
        if isinstance(shard, TensorShardMetadata):
            # Tensor parallel ranks all reference the full layer range, but each
            # rank owns a tensor slice. Live selected-worker memory validation
            # must therefore account for storage per tensor rank instead of
            # multiplying the full model storage by every selected node.
...
def _build_mimo_v25_pro_6bit_preview_state(
    ...
    API-only dashboard sessions can have an empty live topology. For MiMo Pro
    6-bit, expose the corrected Studio1+Studio2 target as two 512GB workers so
    /instance/previews returns executable full-70 instance JSON instead of
    preserving the obsolete 3-node-only assumption.
    """
...
class API:
    ...
    async def get_placement_previews(
        ...
        preview_downloads: Mapping[NodeId, Sequence[DownloadProgress]] = self.state.downloads
